...
const createChannelDivs = (channels) => {
  ...
  filteredChannels.forEach((channel) => {
    ...
    div.addEventListener("click", () => {
      showChannelContentWithLoading(channel.id, channel.title);
...
const showChannelContentWithLoading = (channelId, channelName) => {
  ...
  channelContentDiv.style.width = "60%";
  channelContentDiv.style.height = "60%";
...
const fetchChannelContent = async (
  ...
  channelContentDiv,
  loadingIndicator,
  channelName // Add this parameter
) => {
  try {
...
const displayChannelContent = (
  ...
  channelContentDiv.appendChild(channelNameDiv);
...
const displayAboutContent = () => {
  ...
  aboutContentDiv.style.width = "80%";
  aboutContentDiv.style.height = "80%";
  ...
  setTimeout(() => {
    ...
    `;
    // Insert content after the close button
    closeButton.insertAdjacentHTML("afterend", contentHTML);
    console.log("Content loaded");
  }, 1000);

  console.log("About content displayed");
};
...
const fetchGroupContents = async (groupSlug) => {
  ...
  documentationDiv.className = "channel-content";
  documentationDiv.style.top = "50px";
  documentationDiv.style.left = "50px";
  documentationDiv.style.position = "fixed";
  documentationDiv.style.width = "80%";
  documentationDiv.style.height = "80%";

  const closeButton = createCloseButton(documentationDiv);
  documentationDiv.appendChild(closeButton);

  const loadingIndicator = document.createElement("div");
  loadingIndicator.className = "loading-indicator";
  loadingIndicator.textContent = "Loading...";
  documentationDiv.appendChild(loadingIndicator);

  container.appendChild(documentationDiv);
  documentationDiv.style.display = "block";

  if (window.makeDraggable) {
    window.makeDraggable(documentationDiv);
  }
  setHighestZIndex(documentationDiv);

  documentationDiv.addEventListener("mousedown", () => {
    setHighestZIndex(documentationDiv);
  });
  ...
  try {
    ...
    for (const channel of channelsData.channels) {
      ...
      if (contentsResponse.ok) {
        ...
        if (contentsData.contents.length > 0) {
          // Check if the channel has contents
          allContents.push({
            channel: channel.title,
            channelId: channel.id, // Store the channel ID
    ...
    documentationDiv.removeChild(loadingIndicator);

    createD3Visualization(allContents, documentationDiv, loadingIndicator);
  } catch (error) {
